...
const Ripple = forwardRef(
  (
    ...
    const determineRippleSize = () => {
      ...
      return { size: `${initialSize}px`, scale: rippleScale, duration: dynamicDuration };
    };
    ...
    const getTranslationCoordinates = (event) => {
      ...
      const maxDim = Math.max(height, width);
      const initialSize = Math.floor(maxDim * INITIAL_ORIGIN_SCALE);
      const endPoint = { x: (width - initialSize) / 2, y: (height - initialSize) / 2 };

      let startPoint = event
        ? { x: event.clientX - left, y: event.clientY - top }
        : { x: width / 2, y: height / 2 };

      startPoint = { x: startPoint.x - initialSize / 2, y: startPoint.y - initialSize / 2 };
    ...
    const handlePointerLeave = (event) => {
      if (!shouldReactToEvent(event)) return;
      setIsHovered(false);
      if (stateRef.current !== RippleState.INACTIVE) endPressAnimation();
    };
